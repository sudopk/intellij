...
def _collect_android_ide_info(target, ctx, semantics, ide_info, ide_info_file, output_groups):
    ...
    if android_semantics and hasattr(android_semantics, "get_render_resolve_jar"):
        render_resolve_jar = android_semantics.get_render_resolve_jar(target, ctx)

    if render_resolve_jar:
        update_sync_output_groups(output_groups, "intellij-resolve-render-android", depset([render_resolve_jar]))
