 ...
 * for class files. This only works for resource modules (i.e. not the .workspace module).
 *
 ...
 * This class calculates the binary targets and attempts to find classes from JARs that contain all
 * runtime dependencies of the binary, which includes all runtime dependencies of the resource
 * targets.
 *
...
public class RenderJarClassFileFinder implements BlazeClassFileFinder {
  ...
  // tracks whether the class file finder has been created for workspace module
  ...
  public VirtualFile findClassFile(String fqcn) {
    // resort to fallback finder since we don't support workspace module yet
    ...
    for (int i = 0; i < binaryTargets.size(); i++) {
      VirtualFile virtualFile = getClassFromActiveRenderResolveJar(projectData, fqcn);
      if (virtualFile != null) {
        return virtualFile;
      }
  ...
  /**
   * Returns class file for fqcn if found in the render JAR corresponding to {@link #currentBinary}.
   * Returns null if something goes wrong or if render JAR does not contain fqcn
   */
  @Nullable
  private VirtualFile getClassFromActiveRenderResolveJar(
      BlazeProjectData projectData, String fqcn) {
    ...
    if (renderResolveJarFile == null) {
      LOG.warn("Could not find render resolve jar for " + activeBinaryTarget.getLabel());
      return null;
    }
  ...
  private ImmutableList<TargetKey> collectDependentBinaryTargets() {
    ...
    // Can't calculate binary targets if we don't know the AndroidResourceModule.
    // This will happen for .workspace module
